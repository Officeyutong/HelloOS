...
static size_t displayCharacter(char c, int* a, char* buf) {
    // putchar(c);
    *(buf) = c;
    *a += 1;
    return 1;
...
int svprintf(char* buf, const char* format, va_list list) {
    ...
    const auto displayString = [&](const char* c, int* a) {
        auto t = ::displayString(c, a, buf);
        buf += t;
    };
    const auto displayCharacter = [&](char c, int* a) {
        auto t = ::displayCharacter(c, a, buf);
        buf += t;
    };
        ...
        if (format[i] == '%') {
            ...
            switch (specifier) {
                    ...
                    switch (length) {
                        case 0: {
                            ...
                            __int_str(integer, intStrBuffer, base, plusSign,
                                      spaceNoSign, lengthSpec, leftJustify,
                                      zeroPad);
                ...
                case 'G': {
                    ...
                    __int_str(floating, intStrBuffer, base, plusSign,
...
__attribute__((format(printf, 2, 3))) int sprintf(char* buf,
    ...
    int i = svprintf(buf, format, list);
    va_end(list);
    return i;
}
